echo "PREPROCESS" && TE_detective preprocess -i test_sim.bam -r ref_fofn \
    ...
    && echo 'NADISCOVER' && TE_detective nadiscover -i test_sim.bam -r ref_fofn --polyA --discord_cluster_dens 5  \
    && echo 'ANALYZE' && TE_detective analyze -i test_sim.bam -r ref_fofn --inp initial_predictions.txt \
    && echo 'CLUSTER2D' && TE_detective cluster2d -i test_sim.bam -r ref_fofn \
